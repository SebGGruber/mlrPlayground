...
observe({
  ...
  ##add classification datasets 
    ...
    #6.add Across Spiral data sets
      ...
      x1    = c(r * sin(angle) + noisy_rexp, r * sin(angle + pi) + noisy_rexp)
      x2    = c(r * cos(angle) + noisy_rexp, r * cos(angle + pi) + noisy_rexp)
      class = c(rep("Class 1",amount),rep("Class 2",amount))
    ...
    #7. add Opposite Arc data sets 
      ...
      x1     = c(range1, range2) + noisy_rexp
      x2     = c(sin(range1) + runif(amount,-1,1) / 4,sin(range2+pi/2) + runif(amount,-1,1) / 4) + noisy_rexp
      class  = c(rep("Class 1", amount), rep("Class 2", amount))
  ...
  ### add regression data sets
    ...
    #7. add Circle data sets
      ...
      data.frame(
        x = sqrt(radius) * cos(2 * pi * angle) + noisy_rnom * 2,
        y = sqrt(radius) * sin(2 * pi * angle) + noisy_rnom * 2
      )
  ...
  ### add clustering data sets
    ...
    #2. add Clustering Dataset 2
      ...
      x      = c(radius * sin(angle) + noisy_rexp, radius * sin(angle + pi) + noisy_rexp)
      y      = c(radius * cos(angle) + noisy_rexp, radius * cos(angle + pi) + noisy_rexp)
    ...
    #3. add Clustering Dataset 3
      ...
      y      = c(sin(p1) + runif(amount, -1, 1) / 4,sin(p2+pi/2) + runif(amount, -1, 1) / 4, sqrt(radius) * sin(2*pi*angle)) + noisy_rexp
