...
parameter    = reactiveValues(test_ration = 1, scope = 3)
...
observe({
  ...
  amount       = round(200 * test_ration)

  # by noisy size change data sets
  values$scope = req(input$scope)
  scope  = req(values$scope)
  ...
  data = {
    ...
    if (task == "1.Circle") {
      ...
      y = 2 * (cos(2 * x) + c(1 : amount) / 75 + rexp_scope - 2.1)
      ...
      x = runif(amount, -5, 5)
      y = (8 * sigmoid(3 * x)+ rnom_scope / 4) - 4
      ...
      x = sqrt(radius) * cos(2 * pi * angle) + rnom_scope / 3
      ...
      x = (r * sin(2 * t) + rexp_scope )
      y = (r * cos(2 * t) + rexp_scope ) -1 
      ...
      x  = rexp(amount, 1)%%5
      y1 = sqrt( x) + rnom_scope /2
      y2 = -sqrt( x) + rnom_scope /2
      y  = c(y1,y2)
      ...
      y         = c(y_class1, y_class2, y_class3)
      ...
      x        = sqrt(abs(c(r_class1, r_class2,r_class3,r_class4))) * cos(2*pi*angle) /3
      y        = sqrt(c(r_class1, r_class2,r_class3,r_class4)) * sin(2*pi*angle) /3
      ...
      x4     = (rexp(amount, 1))%%5
      y4     = c(sqrt(4 * x4),-sqrt(4 * x4)) + rnom_scope 
